...
#
# 2024/08/29 ImageMaskDatasetGenerator.py
...
class ImageMaskDatasetGenerator:
  ...
  def __init__(self, 
               input_dir  = "./Brain-MRI-Dataset/", 
               dataset    = "PD25",
               output_dir = "./Brain-MRI-master", 
               angle      = 0, 
               resize     = 512,
               augmentation=True):
    ...
    self.image_file = "sub-" + dataset + "_rez-500um*T2w.nii.gz"
    self.seg_file   = "sub-" + dataset + "_rez-500um*atlas.nii.gz"
...
if __name__ == "__main__":
  try:
    dataset = "PD25"
    datasets = ["ICBM2009asym", "ICBM2009sym", "PD25"] 
    if len(sys.argv) == 2:
      dataset = sys.argv[1]
      print("--- dataset {}".format(dataset))
    if not dataset in datasets:
      raise Exception("Invalid dataaet " + dataset)
    input_dir   = "./sub-" + dataset + "/anat/"
    output_dir  = "./Brain-MRI-master-" + dataset
